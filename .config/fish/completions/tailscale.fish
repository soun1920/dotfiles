...
function __tailscale_targets
    tailscale status --json 2>/dev/null \
        | string match -r '"DNSName":"[^"]*"' \
        | string replace -r '"DNSName":"([^.]*)[^"]*"' '$1'
end

# tailscale file send のターゲット補完
complete -c tailscale -n "__fish_seen_subcommand_from file" -n "__fish_seen_subcommand_from send" \
    -a "(__tailscale_targets)" -d "Tailscale peer"

# サブコマンド補完
complete -c tailscale -f -n "not __fish_seen_subcommand_from file" -a file -d "Send/receive files"
complete -c tailscale -f -n "__fish_seen_subcommand_from file" -a send -d "Send file to peer"
complete -c tailscale -f -n "__fish_seen_subcommand_from file" -a get -d "Receive files"
